...
namespace NVorbis
{
    class VorbisStreamDecoder : IVorbisStreamStatus, IDisposable
    {
        ...
        bool LoadComments(DataPacket packet)
        {
            ...
            var bytes = packet.ReadBytes(packet.ReadInt32());
            _vendor = Encoding.UTF8.GetString(bytes, 0, bytes.Length);

            _comments = new string[packet.ReadInt32()];
            for (int i = 0; i < _comments.Length; i++)
            {
                bytes = packet.ReadBytes(packet.ReadInt32());
                _comments[i] = Encoding.UTF8.GetString(bytes, 0, bytes.Length);
            }
