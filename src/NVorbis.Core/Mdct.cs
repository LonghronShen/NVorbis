    ...
    class Mdct
    {
        ...
        float[] GetBuffer()
        {
            lock (_threadLocalBuffers)
            {
                float[] buf;
#if PORTABLE
                var currentThreadId = System.Threading.Tasks.Task.CurrentId ?? 0;
#else
                var currentThreadId = System.Threading.Thread.CurrentThread.ManagedThreadId;
#endif
                if (!_threadLocalBuffers.TryGetValue(currentThreadId, out buf))
                {
                    _threadLocalBuffers[currentThreadId] = (buf = new float[_n2]);
                }
                return buf;
            }
        }
